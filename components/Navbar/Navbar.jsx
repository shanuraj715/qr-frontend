...
const ExternalLink = ({item}) => {
	return <a href={item.href} target={item.newTab ? '_blank' : "_self"} className="d-flex justify-content-center align-align-items-center">
...
const InternalLink = ({item}) => {
	return <Link href={item.href ?? '#'} className="d-flex justify-content-center align-align-items-center">
		<span className="pe-2 text-white">{item.icon}</span>
		{item.text}
	</Link>
}
        ...
        <div className={'container py-2 d-flex justify-content-between align-items-center '}>
            ...
            <div className='nav-right d-flex justify-content-center align-items-center'>
                ...
                <ul className="d-flex gap-3 gap-md-4 ">
					{list.map(item => {
						return <li key={item.id} className={styles.rightLinks}>
							{item.external ? <ExternalLink item={item} /> : <InternalLink item={item} />}
						</li>
					})}
                    
                </ul>
				<Link href="/login" className={`${styles.loginBtn} ms-4`}>
					<span>
						<BoxArrowInRight />
					</span>
					Login
